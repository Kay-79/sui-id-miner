...
function ActionInstructions({
    ...
    const StepRow = ({
        ...
        <div className="flex items-start gap-2">
            <span
                className={`${badgeColor} text-white text-[10px] px-1.5 py-0.5 rounded font-bold`}
            >
                {step}
            </span>
...
function ResultItem({
    ...
    const resultType =
        result.type === 'GAS_COIN'
            ? 'GAS_COIN'
            : result.type === 'MOVE_CALL'
              ? 'MOVE_CALL'
              : 'PACKAGE'
    const config = RESULT_CONFIG[resultType]

    const displayId =
        result.type === 'GAS_COIN' || result.type === 'MOVE_CALL'
            ? result.objectId
            : result.packageId
            ...
            >
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <span
                            className={`text-xs font-bold px-2 py-0.5 rounded text-white ${config.bgBadge}`}
                        >
                            {config.label}
                        </span>
                ...
                <div className="p-4 pt-0 border-t-2 border-dashed border-gray-300">
                    <div className="pt-3 space-y-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span
                                className={`font-mono text-sm font-bold break-all ${config.textId}`}
                            >
                                {safeId}
                            </span>
                        </div>
